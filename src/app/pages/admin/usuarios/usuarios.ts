...
export class Usuarios implements OnInit {
  ...
  usuariosComuns: UsuarioResponse[] = [];
  usuariosAdmins: UsuarioResponse[] = [];
  ...
  listarUsuarios() {
    ...
    forkJoin({
      admins: this.usuarioService.getUsuariosAdmins(0, 5),
      comuns: this.usuarioService.getUsuariosComuns(0, 5)
    })
    .pipe(
      finalize(() => {
    ...
    .subscribe({
      next: (response) => {
        this.usuariosAdmins = response.admins.content;
        this.usuariosComuns = response.comuns.content;
        console.log('Administradores:', this.usuariosAdmins);
        console.log('Comuns:', this.usuariosComuns);
        this.cdr.detectChanges();
      },
      error: (error) => {
